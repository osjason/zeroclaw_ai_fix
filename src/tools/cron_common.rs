...
pub(crate) fn guard_cron_run_request<'a>(
    config: &Config,
    args: &'a Value,
) -> Result<CronJobRequest<'a>, ToolResult> {
    ensure_cron_enabled(config, "cron_run")?;
    parse_job_request(args)
}
...
pub(crate) fn command_for_job_preflight(job_type: JobType, command: &str) -> Option<&str> {
    match job_type {
        JobType::Shell => Some(command),
        JobType::Agent => None,
    }
}
